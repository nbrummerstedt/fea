...
class MeshRegular {
	...
	unsigned nEx() { return ( xmax-xmin >= ymax-ymin && xmax-xmin >= zmax-zmin) ? R :
							( ymax-ymin >= xmax-xmin && ymax-ymin >= zmax-zmin) ? ceil( (double) R * (xmax-xmin)/(ymax-ymin) ) :
							( zmax-zmin >= xmax-xmin && zmax-zmin >= ymax-ymin) ? ceil( (double) R * (xmax-xmin)/(zmax-zmin) ) : 0 ); }

	unsigned nEy() { return ( xmax-xmin >= ymax-ymin && xmax-xmin >= zmax-zmin) ? ceil( (double) R * (ymax-ymin)/(xmax-xmin) ) :
							( ymax-ymin >= xmax-xmin && ymax-ymin >= zmax-zmin) ? R :
							( zmax-zmin >= xmax-xmin && zmax-zmin >= ymax-ymin) ? ceil( (double) R * (ymax-ymin)/(zmax-zmin) ) : 0 ); }

	unsigned nEz() { return	( xmax-xmin >= ymax-ymin && xmax-xmin >= zmax-zmin) ? ceil( (double) R * (zmax-zmin)/(xmax-xmin) ) :
							( ymax-ymin >= xmax-xmin && ymax-ymin >= zmax-zmin) ? ceil( (double) R * (zmax-zmin)/(ymax-ymin) ) :
							( zmax-zmin >= xmax-xmin && zmax-zmin >= ymax-ymin) ? R : 0 ); }
